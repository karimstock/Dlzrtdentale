#!/usr/bin/env node
// =============================================
// JADOMI — DoctorAI JSON-LD Scraper
//
// Visite les 7432 URLs du sitemap DoctorAI
// et extrait les données produit via JSON-LD structuré
// SANS Puppeteer — HTTP pur, 5 requêtes concurrentes
//
// Usage: node scripts/scrape-doctorai-jsonld.js
// =============================================
...
const PROGRESS_FILE = '/tmp/doctorai-jsonld-progress.json';
const LOG_FILE = '/tmp/doctorai-jsonld.log';
const URLS_FILE = '/tmp/doctorai-urls.txt';
...
async function importBatch(products) {
  ...
  const postData = JSON.stringify({ source: 'doctorai', products: items });
  try {
...
async function main() {
  log('╔══════════════════════════════════════════════════════════╗');
  log('║  DoctorAI JSON-LD SCRAPER — 7432 URLs sitemap          ║');
  ...
  if (fs.existsSync(URLS_FILE)) {
    ...
    urls = [];
    for (let i = 1; i <= 3; i++) {
      const xml = await fetchHTML(`https://www.doctor-ai.fr/media/sitemap_35-9-${i}.xml`);
  ...
  const elapsed = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
  log(`\nDoctorAI JSON-LD TERMINÉ: ${progress.totalProducts + allProducts.length} produits en ${elapsed}min`);

  try {
    await transporter.sendMail({
      from: 'JADOMI Engine <noreply@jadomi.fr>', to: 'karim_bahmed@yahoo.fr',
      subject: `🦷 DoctorAI JSON-LD: ${progress.totalProducts} produits`,
      html: `<h2>DoctorAI Sitemap JSON-LD</h2><p>${progress.totalProducts} produits, ${elapsed} min, ${urls.length} URLs</p>`,
    });
  } catch (e) {}
}
