...
const BACKUP_FILE = '/home/ubuntu/jadomi/tmp/doctorstrong-sitemap-' + new Date().toISOString().slice(0, 10) + '.json';
...
async function fetchSitemapUrls(page) {
  ...
  const sitemapFiles = [
    '/media/sitemap/sitemap_34-8-1.xml',
    '/media/sitemap/sitemap_34-8-2.xml'
  ];
  ...
  try {
    ...
    for (const catUrl of categories) {
      ...
      if (catIdx % 50 === 0) {
        ...
        // Sauvegarder progrès
        fs.writeFileSync('/home/ubuntu/jadomi/tmp/strong-sitemap-progress.json', JSON.stringify(Object.values(allProducts)));
      }
