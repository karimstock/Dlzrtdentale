...
let _interval = null;
// Circuit breaker : si Supabase fail N fois de suite, pause longue
let _consecutiveErrors = 0;
const MAX_ERRORS_BEFORE_PAUSE = 3;
const PAUSE_DURATION = 300000; // 5 minutes

async function safeProcessExpired() {
  try {
    await processExpiredAttempts();
    _consecutiveErrors = 0; // Reset on success
  } catch (e) {
    _consecutiveErrors++;
    if (_consecutiveErrors >= MAX_ERRORS_BEFORE_PAUSE) {
      console.warn(`[GPO Scheduler] ${_consecutiveErrors} erreurs consécutives — pause 5 min`);
      clearInterval(_interval);
      _interval = null;
      setTimeout(() => { _consecutiveErrors = 0; startScheduler(); }, PAUSE_DURATION);
    }
  }
}
...
function startScheduler() {
  if (_interval) return;
  _interval = setInterval(safeProcessExpired, 60000);
  _interval.unref();
  console.log('[GPO Scheduler] Démarré — polling toutes les 60s (circuit breaker actif)');
}
