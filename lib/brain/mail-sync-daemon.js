...
async function syncAccount(account) {
  ...
  try {
    ...
    try {
      // Chercher les mails depuis le dernier sync (ou 3 jours par défaut)
      // Si premier scan → depuis le 1er janvier 2026, sinon depuis dernier scan - 1h
      // Ensuite chaque sync ne prend que les nouveaux mails
    ...
    // ── Scanner les Envoyés pour marquer les mails déjà répondus ──
    try {
      // Trouver le dossier Sent (noms possibles selon provider)
      const folders = await client.list();
      const sentFolder = folders.find(f =>
        f.specialUse === '\\Sent' || /^(Sent|Envoy|Messages envoy)/i.test(f.path)
      );

      if (sentFolder) {
        const sentLock = await client.getMailboxLock(sentFolder.path);
        try {
          const sentSeqs = await client.search({ since });
          const sentBatch = sentSeqs.slice(-50); // derniers 50 envoyés

          if (sentBatch.length > 0) {
            // Collecter les destinataires + sujets des mails envoyés
            const sentMails = [];
            for await (const msg of client.fetch(sentBatch.join(','), { source: true }, { uid: false })) {
              try {
                const parsed = await simpleParser(msg.source);
                sentMails.push({
                  to: parsed.to?.value?.[0]?.address || '',
            ...
            // Matcher avec les mails inbox non répondus
            if (sentMails.length > 0) {
              const { data: unreplied } = await db()
                .from('mails_inbox')
                .select('id, from_address, subject')
                .eq('societe_id', account.societe_id)
                .eq('replied', false)
                .eq('needs_response', true);

              let matched = 0;
              for (const inbox of (unreplied || [])) {
                const inboxSubject = (inbox.subject || '').replace(/^Re:\s*/i, '').toLowerCase().trim();
                const wasReplied = sentMails.some(sent =>
                  // Match par sujet (Re: même sujet) ET destinataire = expéditeur du mail reçu
                  (sent.subject === inboxSubject && sent.to === inbox.from_address)
                );
                if (wasReplied) {
              ...
              if (matched > 0) console.log(`[MAIL-DAEMON] ${matched} mails marqués comme répondus (Envoyés)`);
            }
          }
        } finally { sentLock.release(); }
      }
    } catch (sentErr) {
      // Le scan Envoyés est un bonus — si ça plante, on continue
      console.warn('[MAIL-DAEMON] Sent scan error:', sentErr.message);
    }
...
function startMailDaemon() {
  ...
  console.log('[JADOMI] Mail Daemon démarré (sync toutes les 5 minutes)');
}
