...
const { buildKnowledgeBlock, detectThemes } = require('./code-travail-base');
...
async function enrichWithLegalData(question, dossierContext = null) {
  ...
  // Enrichir avec la base de connaissances pré-chargée (Code du travail + jurisprudence fondamentale)
  const themes = detectThemes(question);
  if (themes.length) {
    const knowledgeBlock = buildKnowledgeBlock(themes);
