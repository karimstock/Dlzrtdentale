...
function validateResponse(text) {
  ...
  // Tutoiement — patterns STRICTS (éviter les faux positifs sur "consulter", "votre", etc.)
  const tutPatterns = [
    /(?:^|\.\s+|\?\s+|\!\s+)tu\s+(?:peux|veux|dois|as|es|fais|vas|sais|aimes|penses|crois)/im,
    /\bton\s+(?:cabinet|mail|compte|agenda|planning|labo|stock|médecin|dentiste)/i,
    /\bta\s+(?:boîte|boite|facture|commande|question|demande|réponse)/i,
    /\btes\s+(?:mails|factures|patients|rendez|documents|commandes)/i,
    /(?:^|\s)t[''](?:as|es|inqui[eè]te|en\s)/i,
  ];
  if (tutPatterns.some(p => p.test(text))) {
