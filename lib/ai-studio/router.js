...
// --- Score Engine (couche optimisation au-dessus des règles métier) ---
...
const KlingProvider = require('./providers/kling');
const ViduProvider = require('./providers/vidu');
...
function initProviders() {
  providers = {
    ...
    'kling': new KlingProvider(process.env.KLING_API_KEY),
    'vidu': new ViduProvider(process.env.VIDU_API_KEY),
...
// --- Sélection déterministe (règles métier fixes, fallback garanti) ---
function selectProvider(type, quality) {
  switch (type) {
    ...
    case 'avatar':
      return getProvider('vidu');
    default:
      throw new Error(`unknown_generation_type: ${type}`);
  }
}

// --- Sélection intelligente (scoring dynamique + fallback déterministe) ---
// Le scoring OPTIMISE le choix parmi les providers éligibles
// Si le scoring échoue ou ne trouve rien → fallback sur selectProvider()
function selectProviderSmart(type, quality, options = {}) {
  // Règles métier : quels providers sont autorisés pour ce type
  const allowedByType = {
    'image':          ['openai-image', 'gemini'],
    'video':          ['remotion', 'vidu', 'kling', 'openai-video'],
    'video_template': ['remotion'],
    'video_wow':      ['vidu', 'kling'],
    'voice':          ['openai-tts', 'elevenlabs'],
    'avatar':         ['kling', 'vidu'],
    'lip-sync':       ['kling', 'vidu'],
    'texte':          ['claude', 'deepseek', 'mistral'],
    'redaction':      ['claude', 'mistral'],
    'tache_simple':   ['deepseek', 'mistral', 'claude'],
    'detourage':      ['removebg', 'gemini'],
    'vision':         ['gemini', 'openai-image'],
    'stock_image':    ['unsplash', 'pexels'],
    'stock_video':    ['pexels'],
  };
  ...
  // Scoring dynamique
  const result = ScoreEngine.score(type, {
    quality: quality || 'standard',
    sensible: options.sensible || false,
    europe_required: options.europe_required || false,
    allowed_providers: allowed,
    blocked_providers: options.blocked_providers || [],
  });

  // Si le scoring a trouvé un provider et qu'on l'a en instance
  if (result.provider) {
    const provider = getProvider(result.provider);
    if (provider) {
      // Attacher les infos de scoring pour le logging
      provider._scoreResult = result;
      return provider;
    }
  }

  // Fallback déterministe (jamais cassé)
  console.warn(`[SCORE] Fallback déterministe pour type=${type}`);
  return selectProvider(type, quality);
}
...
async function generateCreative(supabase, userId, societeId, type, quality, options) {
  // Scoring dynamique si activé, sinon fallback déterministe
  const useSmart = options.smart !== false; // activé par défaut
  const provider = useSmart
    ? selectProviderSmart(type, quality, options)
    : selectProvider(type, quality);
  ...
  try {
    ...
    // Scoring : enregistrer le succès pour améliorer les choix futurs
    ScoreEngine.recordResult(provider.name, type, true, durationMs);

    // Log
    const scoreInfo = provider._scoreResult || null;
    const log = await logGeneration(supabase, {
      ...
      metadata: {
        ...
        score_alternatives: scoreInfo ? scoreInfo.alternatives : null,
      },
    });
    ...
    // Scoring : enregistrer l'échec (pénalise ce provider pour les prochains choix)
    ScoreEngine.recordResult(provider.name, type, false, failDuration);
    ...
    await logGeneration(supabase, {
      ...
      generation_time_ms: failDuration,
    });
