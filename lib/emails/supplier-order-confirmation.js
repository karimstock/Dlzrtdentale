...
// Envoyé après acceptation d'une offre GPO.
// Révèle l'identité du cabinet au fournisseur.
// =============================================
 ...
 * Révèle l'identité du cabinet (cachée pendant la négociation).
 *
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
  ...
  <div style="padding:28px;">
    ...
    <!-- Titre -->
    <h2 style="margin:0 0 4px;font-size:20px;color:#0f172a;">Commande confirmée</h2>
    ...
    <div style="margin:24px 0;padding:20px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;">
      ...
      <div style="font-size:13px;color:#334155;line-height:2;">
        ...
        3. \u00c9mettez votre facture au nom du cabinet<br>
  ...
  const subject = `JADOMI - Commande confirm\u00e9e ${orderNumber} - ${rawCabinetNom}`;
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (result.ok) {
        console.log(`[GPO Email Supplier Confirm] Envoi OK \u00e0 ${supplierEmail}`);
        return result;
      }
  ...
  console.error(`[GPO Email Supplier Confirm] \u00c9chec 3 tentatives pour ${supplierEmail}: ${lastErr}`);
