...
// Factures émises par JADOMI au nom du fournisseur
...
/** Nettoie un nom de fichier (retire tout sauf alphanumérique, tiret, underscore, point) */
...
/**
 * Génère le bloc HTML résumé de facture (réutilisé dans les 2 emails).
 */
  ...
  return `
    <div style="margin:24px 0;padding:20px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
          <td style="padding:6px 0;font-size:13px;color:#64748b;">\u00c9ch\u00e9ance</td>
...
/**
 * Génère le footer JADOMI commun.
 */
function footerHtml() {
  return `
...
/**
 * Génère le header JADOMI commun.
 */
function headerHtml() {
  return `
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
  ...
  <div style="padding:28px;">
    ...
    <div style="margin:24px 0;padding:16px 20px;background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 10px 10px 0;">
      <p style="margin:0;font-size:12px;color:#92400e;line-height:1.6;">
        Cette facture a \u00e9t\u00e9 \u00e9mise par JADOMI au nom et pour le compte de ${safeSupplierName}, conform\u00e9ment au mandat de facturation r\u00e9f. ${safeMandateRef} (art. 289-I-2 CGI).
      </p>
    </div>
    ...
    <div style="margin:24px 0;padding:16px 20px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;">
      ...
      <p style="margin:0;font-size:13px;color:#334155;line-height:1.6;">
        La facture est jointe \u00e0 cet email au format Factur-X.
      </p>
    </div>
    ...
    <div style="margin:24px 0;padding:16px 20px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;">
      ...
      <p style="margin:0;font-size:13px;color:#334155;line-height:1.6;">
        R\u00e8glement \u00e0 effectuer \u00e0 l'ordre de <strong>JADOMI SAS</strong> avant le <strong>${safeDueDate}</strong>.
      </p>
    </div>
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (result.ok) {
        console.log(`[Invoice Email Cabinet] Envoi OK \u00e0 ${cabinetEmail}`);
        return result;
      }
  ...
  console.error(`[Invoice Email Cabinet] \u00c9chec 3 tentatives pour ${cabinetEmail}: ${lastErr}`);
...
/**
 * Envoie une copie de la facture au fournisseur (émise en son nom).
 *
 * @param {Object} options
 ...
 * @param {number} [options.payment_delay=30]       — jours après encaissement
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
  ...
  <div style="padding:28px;">
    ...
    <!-- Titre -->
    <h2 style="margin:0 0 4px;font-size:20px;color:#0f172a;">Facture \u00e9mise en votre nom</h2>
    <p style="margin:0 0 24px;font-size:14px;color:#64748b;">Facture n\u00b0 ${escHtml(inv.number)}</p>
    ...
    <p style="color:#475569;font-size:14px;line-height:1.6;">
      Conform\u00e9ment au mandat de facturation r\u00e9f. <strong>${safeMandateRef}</strong>, JADOMI a \u00e9mis la facture suivante en votre nom et pour votre compte :
    </p>
    ...
    <div style="margin:24px 0;padding:20px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
          <td style="padding:6px 0;font-size:13px;color:#64748b;">Montant \u00e0 reverser</td>
          <td style="padding:6px 0;font-size:13px;color:#166534;font-weight:700;">${safeNet} EUR HT</td>
        </tr>
        <tr>
          <td style="padding:6px 0;font-size:13px;color:#64748b;">Reversement pr\u00e9vu</td>
          <td style="padding:6px 0;font-size:13px;color:#0f172a;">sous ${Number(payment_delay)} jours apr\u00e8s encaissement</td>
        </tr>
      </table>
    </div>
    ...
    <div style="margin:24px 0;padding:16px 20px;background:#fffbeb;border-left:4px solid #f59e0b;border-radius:0 10px 10px 0;">
      <p style="margin:0;font-size:12px;color:#92400e;line-height:1.6;">
        Vous disposez de <strong>${Number(contestation_delay)} jours</strong> pour contester cette facture. Pass\u00e9 ce d\u00e9lai, elle sera r\u00e9put\u00e9e accept\u00e9e.
      </p>
    </div>
  ...
  const subject = `JADOMI \u2014 Copie facture ${inv.number || ''} \u00e9mise en votre nom`;
  ...
  for (let i = 0; i < 3; i++) {
    try {
      const result = await sendMail(mailOptions);
      if (result.ok) {
        console.log(`[Invoice Email Supplier Copy] Envoi OK \u00e0 ${supplierEmail}`);
        return result;
      }
      lastErr = result.error;
    } catch (e) {
  ...
  console.error(`[Invoice Email Supplier Copy] \u00c9chec 3 tentatives pour ${supplierEmail}: ${lastErr}`);
  return { ok: false, error: lastErr };
}
