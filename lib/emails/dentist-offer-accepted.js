 ...
 * @param {string} [opts.orderNumber] - numero de commande (optionnel, backward compat)
...
async function sendDentistAcceptedEmail({ dentistEmail, supplierName, request, finalPrice, orderNumber, supplierDetails }) {
  ...
  const orderNumberHtml = orderNumber
    ? `<div style="background:#f0f4ff;border:1px solid #c7d2fe;border-radius:8px;padding:12px 16px;margin:0 0 16px;text-align:center;">
        <div style="font-size:12px;color:#666;margin-bottom:2px;">Numero de commande</div>
  ...
  const supplierContactHtml = supplierDetails
    ? `<div style="background:#f9f9f7;border-radius:8px;padding:12px 16px;margin:16px 0;">
        <div style="font-size:12px;color:#888;margin-bottom:8px;font-weight:600;">Coordonnees du fournisseur</div>
        <table style="width:100%;font-size:14px;color:#555;">
          ...
          ${supplierDetails.phone ? `<tr><td style="padding:4px 0;font-weight:600;">Tel</td><td>${escHtml(supplierDetails.phone)}</td></tr>` : ''}
        </table>
      </div>`
    : '';
  ...
  const prixVerrouilleHtml = orderNumber
    ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:20px;margin:20px 0;text-align:center;">
        <div style="font-size:12px;color:#666;margin-bottom:4px;">Prix verrouille</div>
  ...
  const prochEtapesHtml = orderNumber
    ? `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:16px;margin:16px 0;">
        <div style="font-size:13px;font-weight:700;color:#92400e;margin-bottom:10px;">Prochaines etapes</div>
        <ol style="margin:0;padding:0 0 0 20px;color:#555;font-size:14px;line-height:2;">
          <li>Le fournisseur va preparer votre commande</li>
          <li>Vous recevrez la livraison a l'adresse de votre cabinet</li>
          <li>Le fournisseur emettra une facture a votre nom</li>
          <li>Confirmez la reception dans votre dashboard JADOMI</li>
        </ol>
      </div>`
    : '';
