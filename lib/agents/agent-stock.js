...
async function findBestPrice(supabase, { productName, ref } = {}) {
  try {
    ...
    const { data: results, error } = await query.order('price_ht', { ascending: true, nullsFirst: false }).limit(30);
    ...
    // Formater les résultats avec prix HT normalisé + TTC (REGLE FONDATEUR)
    const prices = all.map(p => ({
      product_id: p.id,
      product_name: p.product_name,
      reference: p.reference,
      fournisseur: p.supplier_name || 'Non renseigné',
      prix_ht: p.price_ht || (p.price ? Math.round(p.price / 1.20 * 100) / 100 : null),
      prix_ttc: p.price_type === 'ht' ? Math.round(p.price * 1.20 * 100) / 100 : p.price,
    ...
    // Trier par prix HT (comparable entre fournisseurs)
    prices.sort((a, b) => {
      const priceA = a.prix_ht || Infinity;
      const priceB = b.prix_ht || Infinity;
