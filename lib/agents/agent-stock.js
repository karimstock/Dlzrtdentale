...
async function checkLowStock(supabase, societeId) {
  try {
    ...
    for (const p of allProducts) {
      const current = p.quantite || p.stock_actuel || 0;
      const minimum = p.seuil_min || 0;
...
async function findBestPrice(supabase, { productName, ref } = {}) {
  try {
    ...
    // Recherche dans la table comparateur/produits
    let query = supabase.from('produits_stock').select('*');

    if (ref) {
      // Recherche exacte par référence
      query = query.or(`reference.eq.${ref},code_ean.eq.${ref},ref.eq.${ref}`);
    }

    if (productName && !ref) {
      // Recherche par nom (ilike)
      query = query.ilike('nom', `%${productName}%`);
    }
    ...
    const prices = all.map(p => ({
      ...
      product_name: p.nom || p.designation || p.name,
      reference: p.reference || p.ref || p.code_ean,
      fournisseur: p.fournisseur || p.supplier || 'Non renseigné',
      prix_catalogue: p.prix_catalogue || p.prix_public || p.prix || null,
      prix_promo: p.prix_promo || p.prix_contrat || p.prix_remise || null,
      prix_unitaire: p.prix_unitaire || null,
      remise_percent: null,
      conditionnement: p.conditionnement || p.packaging || null,
      url: p.url || p.lien || null,
      last_updated: p.updated_at || p.date_maj || null
    }));
