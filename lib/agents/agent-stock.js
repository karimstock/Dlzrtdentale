...
async function findBestPrice(supabase, { productName, ref } = {}) {
  try {
    ...
    const { data: results, error } = await query.order('price_ttc', { ascending: true, nullsFirst: false }).limit(30);
    ...
    // Formater les résultats en TTC (dentistes = non assujettis TVA, TTC = vrai prix)
    const prices = all.map(p => ({
      product_id: p.id,
      product_name: p.product_name,
      reference: p.reference,
      fournisseur: p.supplier_name || 'Non renseigné',
      prix: p.price_ttc || (p.price_type === 'ht' ? Math.round(p.price * 1.20 * 100) / 100 : p.price),
    ...
    // Trier par prix TTC (dentistes non assujettis TVA = TTC est le vrai prix)
    prices.sort((a, b) => {
