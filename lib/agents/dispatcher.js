...
// Singleton Supabase pour le dispatcher (pas de re-création par événement)
let _dispatcherDb = null;
function dispatcherDb() {
  if (!_dispatcherDb) {
    const { createClient } = require('@supabase/supabase-js');
    _dispatcherDb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_KEY);
  }
  return _dispatcherDb;
}
