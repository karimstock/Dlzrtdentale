...
function l(m){document.title='DGDP:'+Object.keys(P).length+'sr';console.log('[DGDP '+Math.round((Date.now()-T)/1000)+'s] '+m);}
    ...
    fetch(catUrls[ci]).then(function(r){return r.text();}).then(function(h){
      ...
      d.querySelectorAll('[onclick]').forEach(function(el){
        ...
        if(m&&!seenProd[m[1]]){
          seenProd[m[1]]=true;
          var url=m[1];
          if(url.indexOf('http')!==0)url='https://www.prothesiste.dentalgooddeal.com/'+url.replace(/^\//,'');
...
function finish(){
  ...
  a2.download='dgd-prothesiste-'+pr.length+'.json';
  a2.click();
...
// LANCEMENT
l('=== DENTALGOODDEAL PROTHESISTE v1 ===');
collectCats();
collectProducts(function(){scrapeProducts();});

window.dgdp_status=function(){l(qi+'/'+productUrls.length+' fiches, '+Object.keys(P).length+' sous-refs, '+totalProducts+' produits');};
window.dgdp_stop=function(){done=true;finish();};
})();
