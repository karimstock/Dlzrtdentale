    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:Inter,Arial,sans-serif;max-width:500px;margin:0 auto;padding:32px;">
          <h2 style="font-size:20px;color:#0A1628;margin-bottom:16px;">Code de vérification</h2>
          <p style="font-size:14px;color:#5C5C70;margin-bottom:24px;">
            Voici votre code d'accès sécurisé :
          </p>
          ...
          <div style="margin-top:24px;padding-top:16px;border-top:1px solid #E5E2DB;font-size:11px;color:#8E8E9E;">
            Espace sécurisé &middot; Chiffrement AES-256 &middot; Hébergé en France
          </div>
        </div>
      `
    });
...
async function sendSmsOTP(telephone, code) {
  ...
  if (!account || !login) {
    ...
    return { success: false, error: 'SMS non configuré. Utilisez email ou WhatsApp.', fallback: 'email' };
  }

  try {
    const res = await fetch(`https://www.ovh.com/cgi-bin/sms/http2sms.cgi?account=${account}&login=${login}&password=${password}&from=JADOMI&to=${telephone.replace(/\s/g, '')}&message=${encodeURIComponent('JADOMI - Votre code de vérification : ' + code + ' (valable 5 min)')}&noStop=1`);
    const text = await res.text();
    if (text.includes('OK')) return { success: true };
    return { success: false, error: 'Envoi SMS échoué : ' + text };
  } catch (err) {
    return { success: false, error: err.message };
  }
}
...
async function sendSms(telephone, message) {
  ...
  if (!account || !login) {
    ...
    return { success: false, error: 'SMS non configuré.', fallback: 'email' };
  }
...
async function sendWhatsAppOTP(telephone, code) {
  ...
  if (!token || !phoneId) {
    ...
    return { success: false, error: 'WhatsApp non configuré. Utilisez email ou SMS.', fallback: 'email' };
  }
  ...
  try {
    ...
    return { success: false, error: data.error?.message || 'Envoi WhatsApp échoué' };
  } catch (err) {
    return { success: false, error: err.message };
  }
}
